...
const ROWS: Row[] = [
  { key: "role", kind: "string", value: "Full-Stack Developer" },
  { key: "interests", kind: "array", items: ["Applied AI", "Distributed Systems", "Product Engineering"] },
  { key: "approach", kind: "string", value: "Think \u2192 Build \u2192 Test \u2192 Iterate" },
  { key: "status", kind: "string", value: "open to work" },
];

export default function HeroPanel() {
  // Rough delay budget so later lines still stagger in even though the array takes multiple lines.
  let delayIndex = 0;
          ...
          {ROWS.map((row) => {
            ...
            return (
              ...
              >
                <span className="text-signal">{row.key}</span>
                <span className="text-text-faint">: </span>
                <span className="text-link">&quot;{row.value}&quot;</span>
                <span className="text-text-faint">,</span>
              </motion.p>
            );
          })}
